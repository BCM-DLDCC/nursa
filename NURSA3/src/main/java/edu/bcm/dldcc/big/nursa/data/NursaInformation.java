...
public class NursaInformation {
	...
	public void updateReagents() {
		// init vars
		setReagentBreakdown(new PieChartModel());
		setReagentBreakdownMap(new HashMap<String, Long>());
		setReagentTotal(0L);
		...
		// get data
		Root<Reagent> reagentRoot = cq.from(Reagent.class);
		cq.multiselect(reagentRoot.get(Reagent_.type), cb.count(reagentRoot));
		cq.groupBy(reagentRoot.get(Reagent_.type));

		// assign it to our return variables
		// getReagentBreakdown is called instead of setXXX because we need to
		// call
		// a method on the underlying object to set the value on it
		for (Tuple tuple : objectEntityManager.createQuery(cq).getResultList()) {
			getReagentBreakdown().set((String) tuple.get(0),
					(Long) tuple.get(1));
			getReagentBreakdownMap().put((String) tuple.get(0),
					(Long) tuple.get(1));
			setReagentTotal(getReagentTotal() + (Long) tuple.get(1));
	...
	public void updateTranslational() {
		setTranslationalBreakdown(new PieChartModel());
		setTranslationalBreakdownMap(new HashMap<String, Long>());
		setTranslationalTotal(0L);
		
		CriteriaBuilder cb = objectEntityManager.getCriteriaBuilder();

		CriteriaQuery<Tuple> cq = cb.createTupleQuery();

		Root<Translational> translationalRoot = cq.from(Translational.class);
		cq.multiselect(translationalRoot.get(Translational_.type),
				cb.count(translationalRoot));
		cq.groupBy(translationalRoot.get(Translational_.type));

		for (Tuple tuple : objectEntityManager.createQuery(cq).getResultList()) {
			getTranslationalBreakdown().set((String) tuple.get(0), (Long) tuple.get(1));
			getTranslationalBreakdownMap().put((String) tuple.get(0), (Long) tuple.get(1));
			setTranslationalTotal(getReagentTotal() + (Long) tuple.get(1));
		}
	}
